...
class ReleasedPetsFilter extends React.Component {
  constructor (props) {
    super(props)
    this.state = {
      dateFrom: null,
  ...
  componentWillUnmount () {
    store.dispatch({ type: 'RESET_THE_RELEASED_PETS_FILTER_PARAMS' })
  }

  filterPets = () => {
    const hiddenButton = document.getElementById('filterReleasedPetsByDate')
    if (hiddenButton) {
      hiddenButton.click()
    }
  }
  ...
  setDateFrom = (date) => {
    ...
    store.dispatch({ type: 'SET_THE_RELEASED_PETS_FILTER_DATE_FROM', payload: shortDateFrom })
  }

  setDateTo = (date) => {
    this.setState({ dateTo: date })
    const shortDateTo = date ? convertToShortDate(date, 'y-m-d') : null
    store.dispatch({ type: 'SET_THE_RELEASED_PETS_FILTER_DATE_TO', payload: shortDateTo })
  }
  ...
  getComponent = () => {
    ...
    const filterText = this.context.intl.formatMessage({
      id: `${config.labelBasePath}.main.filter_released_pets_by_date`,
      defaultMessage: `${config.labelBasePath}.main.filter_released_pets_by_date`
    })
    const component = (
...
ReleasedPetsFilter.contextTypes = {
...
export default connect(mapStateToProps)(ReleasedPetsFilter)
