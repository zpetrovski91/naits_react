...
class IncomingTransferFilter extends React.Component {
  constructor (props) {
    super(props)
    this.state = {
      tagType: null,
  ...
  componentDidMount () {
    ...
    store.dispatch({ type: 'SET_THE_INCOMING_TRANSFER_FILTER_TAG_TYPE', payload: '7' })
  }

  componentWillUnmount () {
    store.dispatch({ type: 'RESET_THE_INCOMING_TRANSFER_FILTER_PARAMS' })
  }

  filterIncomingTransfers = () => {
    const hiddenButton = document.getElementById('filterIncomingTransfers')
    if (hiddenButton) {
      hiddenButton.click()
    }
  }
  ...
  onChange = e => {
    ...
    if (strcmp(e.target.name, 'tagType')) {
      store.dispatch({ type: 'SET_THE_INCOMING_TRANSFER_FILTER_TAG_TYPE', payload: e.target.value || null })
    } else if (strcmp(e.target.name, 'startTagId')) {
      store.dispatch({ type: 'SET_THE_INCOMING_TRANSFER_FILTER_START_TAG_ID', payload: e.target.value || null })
    } else if (strcmp(e.target.name, 'endTagId')) {
      store.dispatch({ type: 'SET_THE_INCOMING_TRANSFER_FILTER_END_TAG_ID', payload: e.target.value || null })
    }
  }
  ...
  setDateFrom = (date) => {
    ...
    store.dispatch({ type: 'SET_THE_INCOMING_TRANSFER_FILTER_DATE_FROM', payload: shortDateFrom })
  }

  setDateTo = (date) => {
    this.setState({ dateTo: date })
    const shortDateTo = date ? convertToShortDate(date, 'y-m-d') : null
    store.dispatch({ type: 'SET_THE_INCOMING_TRANSFER_FILTER_DATE_TO', payload: shortDateTo })
  }
  ...
  getComponent = () => {
    ...
    const filterText = this.context.intl.formatMessage({
      id: `${config.labelBasePath}.main.filter_incoming_transfers`,
      defaultMessage: `${config.labelBasePath}.main.filter_incoming_transfers`
    })

    return (
      <div
        id='filterIncomingTransfersContainer'
        className={style.container}
        style={{ display: 'flex', flexDirection: 'row', justifyContent: 'start', alignItems: 'center' }}
      >
        <div className={style.title}>{filterText}</div>
        <div id='filterIncomingTransfersInputsContainer' className={style.customContainer}>
          <div>
        ...
        <div
          ...
          onClick={() => this.filterIncomingTransfers()}>
          <img
            id='filter_img'
  ...
  render () {
    ...
    const parentContainer = document.getElementById('action-list')
...
IncomingTransferFilter.contextTypes = {
...
export default connect(mapStateToProps)(IncomingTransferFilter)
