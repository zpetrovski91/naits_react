...
class TerminatedPetsFilter extends React.Component {
  constructor (props) {
    super(props)
    this.state = {
      dateFrom: null,
  ...
  componentWillUnmount () {
    store.dispatch({ type: 'RESET_THE_TERMINATED_PETS_FILTER_PARAMS' })
  }

  filterPets = () => {
    const hiddenButton = document.getElementById('filterTerminatedPetsByDate')
    if (hiddenButton) {
      hiddenButton.click()
    }
  }
  ...
  setDateFrom = (date) => {
    ...
    store.dispatch({ type: 'SET_THE_TERMINATED_PETS_FILTER_DATE_FROM', payload: shortDateFrom })
  }

  setDateTo = (date) => {
    this.setState({ dateTo: date })
    const shortDateTo = date ? convertToShortDate(date, 'y-m-d') : null
    store.dispatch({ type: 'SET_THE_TERMINATED_PETS_FILTER_DATE_TO', payload: shortDateTo })
  }
  ...
  getComponent = () => {
    ...
    const filterText = this.context.intl.formatMessage({
      id: `${config.labelBasePath}.main.filter_terminated_pets_by_date`,
      defaultMessage: `${config.labelBasePath}.main.filter_terminated_pets_by_date`
    })
    const component = (
      <div id='filterPetsContainer' className={style.container}>
        <div className={style.title}>{filterText}</div>
        <div id='filterPetsInputsContainer' className={style.customContainer}>
...
TerminatedPetsFilter.contextTypes = {
...
export default connect(mapStateToProps)(TerminatedPetsFilter)
