...
L.Control.Coordinates = L.Control.extend({
  options: {
    ...
    // Changed default boolean, originally false, setting map center to user coordinates while keeping the scales ... Igi 26012017
  ...
  /**
	*	Called on each keyup except ESC
		...
		Input values are now crs projected, create point and unproject them to generate a latlng pair for setView(). Works as long as map.crs is default(EPSG:4326)
		Once map is required to be in state crs this needs revision
		Igi 22082017
	*/
  _handleSubmit: function () {
    //
    var x = this._inputX.value
    var y = this._inputY.value
    ...
    if (x !== undefined && y !== undefined) {
      ...
      var xyPoint = L.point(x, y)
      var ll = this.options.crs.projection.unproject(xyPoint)
      // var ll = new L.LatLng(y, x);
      // removed add to map function, no need for marker on map, centering set to default on line 26... Igi 26012017
  ...
  _createCoordinateLabel: function (ll) {
    ...
    // Added May 2017 Igi, display crs specified coordinates on map
	    let point = opts.crs.projection.project(ll)
...
*	Returns a Number according to options (DMS or decimal)
  ...
  collapse: function () {
    if (!this._showsCoordinates) {
      ...
      if (this._marker) {
        ...
        // _ordinateLabel is not a function, used for labeling popup? on marker ... removed marker addTo - map- .... Igi 26012017
  ...
  /**
	*	Mousemove callback function updating labels and input elements
		...
		Reproject labels to show state crs (inputX.value, inputY.value). These are shown when the Coordinates container is expanded for user input
		Igi 22082017
	*/
  _update: function (evt) {
    ...
    if (pos) {
      ...
      this._currentPos = pos
	    	let posProj = opts.crs.projection.project(pos)
      this._inputY.value = posProj.y		// L.NumberFormatter.round(pos.lat, opts.decimals, opts.decimalSeperator);
      this._inputX.value = posProj.x		// L.NumberFormatter.round(pos.lng, opts.decimals, opts.decimalSeperator);
      // this._inputY.value = L.NumberFormatter.round(pos.lat, opts.decimals, opts.decimalSeperator);
      // this._inputX.value = L.NumberFormatter.round(pos.lng, opts.decimals, opts.decimalSeperator);
      this._label.innerHTML = this._createCoordinateLabel(pos)	// Already projecting in _createCoordinateLabel, pass LL here
    }
  },
