...
class FinishedMovementDocumentsFilter extends React.Component {
  constructor (props) {
    super(props)
    this.state = {
      dateFrom: null,
  ...
  componentWillUnmount () {
    store.dispatch({ type: 'RESET_THE_FINISHED_MOVEMENT_DOCUMENTS_FILTER_PARAMS' })
  }

  filterFinishedMovementDocuments = () => {
    const hiddenButton = document.getElementById('filterMovementDocumentsByDate')
    if (hiddenButton) {
      hiddenButton.click()
    }
  }
  ...
  setDateFrom = (date) => {
    ...
    store.dispatch({ type: 'SET_THE_FINISHED_MOVEMENT_DOCUMENTS_FILTER_DATE_FROM', payload: shortDateFrom })
  }

  setDateTo = (date) => {
    this.setState({ dateTo: date })
    const shortDateTo = date ? convertToShortDate(date, 'y-m-d') : null
    store.dispatch({ type: 'SET_THE_FINISHED_MOVEMENT_DOCUMENTS_FILTER_DATE_TO', payload: shortDateTo })
  }
  ...
  getComponent = () => {
    ...
    const filterText = this.context.intl.formatMessage({
      id: `${config.labelBasePath}.main.filter_finished_movement_documents_by_date`,
      defaultMessage: `${config.labelBasePath}.main.filter_finished_movement_documents_by_date`
    })
    const component = (
      <div id='filterFinishedMovementDocumentsContainer' className={style.container}>
        <div className={style.title}>{filterText}</div>
        <div id='filterFinishedMovementDocumentsInputsContainer' className={style.customContainer}>
...
FinishedMovementDocumentsFilter.contextTypes = {
...
export default connect(mapStateToProps)(FinishedMovementDocumentsFilter)
