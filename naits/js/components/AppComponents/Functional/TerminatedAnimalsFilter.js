...
class TerminatedAnimalsFilter extends React.Component {
  constructor (props) {
    super(props)
    this.state = {
      dateFrom: null,
  ...
  componentWillUnmount () {
    store.dispatch({ type: 'RESET_THE_TERMINATED_ANIMALS_FILTER_PARAMS' })
  }

  filterAnimals = () => {
    const hiddenButton = document.getElementById('filterTerminatedAnimalsByDate')
    if (hiddenButton) {
      hiddenButton.click()
    }
  }
  ...
  setDateFrom = (date) => {
    ...
    store.dispatch({ type: 'SET_THE_TERMINATED_ANIMALS_FILTER_DATE_FROM', payload: shortDateFrom })
  }

  setDateTo = (date) => {
    this.setState({ dateTo: date })
    const shortDateTo = date ? convertToShortDate(date, 'y-m-d') : null
    store.dispatch({ type: 'SET_THE_TERMINATED_ANIMALS_FILTER_DATE_TO', payload: shortDateTo })
  }
  ...
  getComponent = () => {
    ...
    const filterText = this.context.intl.formatMessage({
      id: `${config.labelBasePath}.main.filter_terminated_animals_by_date`,
      defaultMessage: `${config.labelBasePath}.main.filter_terminated_animals_by_date`
    })
    const component = (
      <div id='filterAnimalsContainer' className={style.container}>
        <div className={style.title}>{filterText}</div>
        <div id='filterAnimalsInputsContainer' className={style.customContainer}>
...
TerminatedAnimalsFilter.contextTypes = {
...
export default connect(mapStateToProps)(TerminatedAnimalsFilter)
